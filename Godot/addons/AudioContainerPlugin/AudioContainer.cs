...
public partial class AudioContainer : Node
{
    ...
    [Export]
    public Node SoundParent { get; set; }
    ...
    public async override void _Ready()
    {
        ...
        for (int i = 0; i < GetChildCount(); i++)
        {
            ...
            if (SoundParent != null)
            {
                CallDeferred("remove_child", n);
                SoundParent.CallDeferred("add_child", n);
                n.SetDeferred(PropertyName.Owner, SoundParent);// Owner = _parent;
                // await ToSignal(n, Node.SignalName.ChildEnteredTree);
            }
            InitialisePlayer(n);
        }
    ...
    private async void InitialisePlayer(Node n)
    {
    ...
    public new void AddChild(Node n, bool forceReadableName = false, InternalMode @internal = InternalMode.Disabled)
    {
        if (SoundParent == null)
        {
            base.AddChild(n, forceReadableName, @internal);
        }
        else
        {
            SoundParent.AddChild(n, forceReadableName, @internal);
        }
        InitialisePlayer(n);
    }
    ...
    public void AdjustVolumeTo(float dB)
    {
        TweenVolume(dB);
    }
